...
public class ExceptionInfoDAOImpl extends SqlMapClientDaoSupport implements ExceptionInfoDAO {
	...
	public ExceptionInfoVO insertExceptionInfo(ExceptionInfoVO exceptionInfoVO) {
		if(exceptionInfoVO == null) 
			return null;
		Integer key = (Integer) getSqlMapClientTemplate().insert("insertExeceptionDetails", exceptionInfoVO);
		if(key == null) 
			return null;
	...
	public List<ExceptionInfoVO> findExceptions(
		...
		return getSqlMapClientTemplate().queryForList("findExceptions", params, skipResults, maxResults);
	}
