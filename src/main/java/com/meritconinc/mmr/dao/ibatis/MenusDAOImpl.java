...
public class MenusDAOImpl extends SqlMapClientDaoSupport implements MenusDAO {
	...
	/**
	 * Returns the menu items that have the same parent as 'action' and belong to a group 
	 * of 'roles'. 
	 * 
	 * @param action
	 * @param roles
	 * @return
	 */
	public List<MenuItemVO> getGroupMenusForAction(String action, Collection<String> roles,  String locale)  {
		Map map = new HashMap();
		map.put("action", action);
		map.put("roles", roles);
		map.put("locale", locale);
		List<MenuItemVO> groupMenus = getSqlMapClientTemplate().queryForList("getGroupMenusForAction", map);
		groupMenus = Common.removeDuplicate(groupMenus, (Comparator)new MenuItemVOComparator(), true);
		return groupMenus;
	}

	/**
	 * Returns the menu items that have the same parent as 'menuId' and belong to a group 
	 * of 'roles'. 
	 * 
	 * @param menuId
	 * @param roles
	...
	/**
	 * Returns the first child in the list for a node – 'menuId'and and belonging to a group 
	 * of 'roles'. 
	 *  
	 * @param menuId
	 * @param roles
	...
	public List<MenuItemVO> getGroupMenusForIdByLevel(int menuId, String level, Collection<String> roles, String locale) {
		// TODO Auto-generated method stub
		Map map = new HashMap();
		map.put("menuId", menuId);
		map.put("level", level);
		...
		map.put("locale", locale);
		map.put("businessId", getBusinessIdForMenuItems());
		List<MenuItemVO> groupMenus = getSqlMapClientTemplate().queryForList("getGroupMenusForIdByLevel", map);
		groupMenus = Common.removeDuplicate(groupMenus, (Comparator)new MenuItemVOComparator(), true);	
		return groupMenus;
	}
	
	private int getBusinessIdForMenuItems(){
		Business business = (Business)ActionContext.getContext().getSession().get(ShiplinxConstants.BUSINESS);
		int business_id = 1; //this should not happen, the business should have been determined based on the URL used to access the site, or through cookies already created
		if(business != null)
