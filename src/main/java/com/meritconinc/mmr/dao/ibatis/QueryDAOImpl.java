...
public class QueryDAOImpl extends SqlMapClientDaoSupport implements QueryDAO {
	...
	public QueryResultVO executeQuery(QueryVO query) throws DAOException {		
		if ( query  == null ) throw new NullPointerException("cannot execute null query");
		if ( query.getSql() == null ) throw new NullPointerException("cannot execute query with null sql");
		if ( query.getParameters() == null ) throw new NullPointerException("cannot execute query with null parameters");
		...
		try {
			...
			//set the max rows for the query. If query max rows is not set (or is zero), then use system default from PROPERTY
			if(query.getMaxRows() <=0){
				int default_max_rows = Integer.parseInt(WebUtil.getProperty(
					Constants.SYSTEM_SCOPE, Constants.QUERY_TOOL_DEFAULT_MAX_ROWS));
	...
	public List<QueryVO> getQueriesByUser(String userId) {
		...
		List<QueryVO> result = (List<QueryVO>)getSqlMapClientTemplate().queryForList("selectQueriesByUser", paramObj);
		return result;
	}
	...
	public List<QueryParameterVO> getQueryParameters(int queryId) {
		...
		List<QueryParameterVO> result = (List<QueryParameterVO>)getSqlMapClientTemplate().queryForList("selectParametersByQuery", paramObj);
		return result;
	}
	...
	public void saveQuery(QueryVO query) {
		if ( query == null ) throw new NullPointerException("can't save a null query.");
		if ( query.getId() == -1 ) {
			insertQuery(query);
		} else {
			updateQuery(query);
	...
	public List<RoleVO> getAuthorizedRolesForQuery(int queryId) {
		...
		List<RoleVO> result = (List<RoleVO>)getSqlMapClientTemplate().queryForList("selectAuthorizedRolesByQuery", paramObj);
		return result;
	}
	...
	public List<User> getAuthorizedUsersForQuery(int queryId) {
		Map<String, Object> paramObj = new HashMap<String, Object>(1);
		paramObj.put("queryId", queryId);
		List<User> result = (List<User>)getSqlMapClientTemplate().queryForList("selectAuthorizedUsersByQuery", paramObj);
		return result;
	}
	...
	public List<RoleVO> getUnauthorizedRolesForQuery(int queryId) {
		Map<String, Object> paramObj = new HashMap<String, Object>(1);
		paramObj.put("queryId", queryId);
		List<RoleVO> result = (List<RoleVO>)getSqlMapClientTemplate().queryForList("selectUnauthorizedRolesByQuery", paramObj);
		return result;
	}
	...
	public List<User> getUnauthorizedUsersForQuery(int queryId) {
		Map<String, Object> paramObj = new HashMap<String, Object>(1);
		paramObj.put("queryId", queryId);
		List<User> result = (List<User>)getSqlMapClientTemplate().queryForList("selectUnauthorizedUsersByQuery", paramObj);
		return result;
	}
