...
public class UserServiceImpl implements UserService {
	...
	public void changePassword(String username, String existingPassword,
			String newPassword, String changedBy, boolean isAdmin)
			throws InvalidPasswordException, SameOldPasswordException, RepeatedPasswordException {
		...
		int pass_hist_count = Integer.valueOf(WebUtil.getProperty(Constants.SYSTEM_SCOPE, Constants.PASSWORD_HISTORY_COUNT)).intValue();
	...
	public void addRequestAccess(final User user)
	...
	protected String createNewAccessEmailBody(User user) {
		...
		String body = MessageUtil.getMessage("message.request.body", user.getLocale()) + " "  + MessageUtil.getMessage("project.title", user.getLocale()) + " site.\n\n";
		body += MessageUtil.getMessage("label.user.name", user.getLocale()) + ": " + user.getUsername() + "\n";
		body += MessageUtil.getMessage("label.first.name", user.getLocale()) + ": " + user.getFirstName() + "\n";
		...
		body += MessageUtil.getMessage("label.occupation", user.getLocale()) + ": " + user.getOccupation() + "\n";
		body += MessageUtil.getMessage("label.email", user.getLocale()) + ": " + user.getEmail() + "\n";
		body += MessageUtil.getMessage("label.phone.number", user.getLocale()) + ": " + user.getPhoneNumber() + "\n";
		if(!StringUtil.isEmpty(phoneExt)){
			body += MessageUtil.getMessage("label.phone.ext", user.getLocale()) + ": " + user.getPhoneNumberExt() + "\n";
		}
		...
		if (!StringUtil.isEmpty(country)) {
			body += MessageUtil.getMessage("label.country.code", user.getLocale())
			+ ": " + country + "\n";
		}
		if (!StringUtil.isEmpty(province)) {
			body += MessageUtil.getMessage("label.state.province", user.getLocale())
			+ ": " + province + "\n";
		}
		
		if (!StringUtil.isEmpty(address)) {
			body += MessageUtil.getMessage("label.address.line", user.getLocale())
			+ ": " + address + "\n";
		}
		if (!StringUtil.isEmpty(city)) {
			body += MessageUtil.getMessage("label.city", user.getLocale())
			+ ": " + city + "\n";
		}
		if (!StringUtil.isEmpty(postalZip)) {
			body += MessageUtil.getMessage("label.postal.zip", user.getLocale())
			+ ": " + postalZip + "\n";
		}
		body += "\n";
		return body;
...
//		}
	...
	public List<User> findUsers(UserSearchCriteria criteria, int startIndex,
			int endIndex) {
		List<User> users = siteUserDAO.findUsers(criteria, startIndex,
					endIndex);
...
//	
	...
	public void sendApprovedEmail(User user) {
		...
		String subject = MessageUtil.getMessage(
				"message.approve.status.subject", locale);
		String body = MessageUtil.getMessage(
				"message.approve.status.message", locale);
		body += "\n\n";
		
		try {
			MailHelper.sendEmailNow2("smtp.pathcom.com", "info@eshipper.com", "Ofnipath!@", user.getBusiness().getName(), 587, "riz@shaw.ca", "rizwan.merchant@meritconinc.com", null,subject, body, null, true);
	...
	private void sendPasswordReminderEmail(User user, String newPassword) {
		...
		try {
			MailHelper.sendEmailNow2(user.getBusiness().getSmtpHost(), user.getBusiness().getSmtpUsername(), user.getBusiness().getSmtpPassword(), user.getBusiness().getName(), user.getBusiness().getSmtpPort(), user.getBusiness().getAddress().getEmailAddress(), user.getEmail(), null, subject, body, null, true);
		} catch (MessagingException e) {
			log.error("Error sending email: " + e.getMessage());
		}
