...
public class ShipWebServiceClient  extends FedExRequestHelper{  
	...
	public ProcessShipmentReply sendRequest(ProcessShipmentRequest request,String SequenceNumber) {  
		...
		request.getRequestedShipment().getRequestedPackageLineItems(0).setSequenceNumber(new PositiveInteger(SequenceNumber));
		logger.debug("Ship request for service: " + request.getRequestedShipment().getServiceType().getValue() + ", using account #: " + this.request.getClientDetail().getAccountNumber());
	...
	public void setOrder() {
	    ...
	    { 
	    	if(order.getPackageTypeId().getPackageTypeId()==ShiplinxConstants.PACKAGE_TYPE_ENVELOPE || order.getPackageTypeId().getPackageTypeId()==ShiplinxConstants.PACKAGE_TYPE_PAK || order.getCustomsInvoice()==null || order.isCustomsInvoiceRequired()==false)
	    	{
...
//		        commodities[0].setHarmonizedCode("0000.");
	    	...
	    	{	
				...
				while(i.hasNext())
				{
					...
					commodities[current].setNumberOfPieces(new NonNegativeInteger(String.valueOf(p.getProductQuantity())));
			        ...
			        commodities[current].setQuantity(new NonNegativeInteger((new Integer(p.getProductQuantity()).toString())));
			        commodities[current].setQuantityUnits("EA");
			        commodities[current].setUnitPrice(new Money());
			        commodities[current].getUnitPrice().setAmount(new java.math.BigDecimal(p.getProductUnitPrice()));
			        commodities[current].getUnitPrice().setCurrency(order.getCustomsInvoice().getCurrency());
			        commodities[current].setCustomsValue(new Money());
			        commodities[current].getCustomsValue().setAmount(new java.math.BigDecimal(p.getProductUnitPrice()*p.getProductQuantity()));
	    	...
	    	if(order.getCustomsInvoice()!=null){
		    	if(!StringUtil.isEmpty(order.getCustomsInvoice().getExportData())){
		    		...
		    		requestedShipment.getCustomsClearanceDetail().getExportDetail().setExportComplianceStatement(order.getCustomsInvoice().getExportData());
				}
				...
				if(!StringUtil.isEmpty(order.getCustomsInvoice().getBillTo())){
					...
					{
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().setPaymentType(PaymentType.SENDER);
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().setPayor(new Payor());
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().getPayor().setAccountNumber(customerCarrier.getAccountNumber1());
						order.getCustomsInvoice().setBillToAccountNum(customerCarrier.getAccountNumber1());
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().getPayor().setCountryCode(customerCarrier.getCountry());
					}	
					...
					{	
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().setPaymentType(PaymentType.THIRD_PARTY);
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().setPayor(new Payor());
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().getPayor().setAccountNumber(order.getCustomsInvoice().getBillToAccountNum());
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().getPayor().setCountryCode(order.getCustomsInvoice().getBillToAddress().getCountryCode());
					}
					else{ //receiver
						requestedShipment.getCustomsClearanceDetail().getDutiesPayment().setPaymentType(PaymentType.RECIPIENT);							
					}
				}	
        ...
        for(int rp=0; rp<rplis.length; rp++){
        	...
        	rpli.setWeight(new Weight(WeightUnits.LB, new BigDecimal(order.getPackages().get(rp).getWeight())));
		...
		logger.debug("---shippingOrder.getHoldForPickupRequired()---------"+order.getHoldForPickupRequired());
...
//		}
		...
		//for express freight
		if(order.getService().getCode().equalsIgnoreCase(SERVICE_INTERNATIONAL_PRIORITY_FREIGHT) || order.getService().getCode().equalsIgnoreCase(SERVICE_INTERNATIONAL_ECONOMY_FREIGHT) ){
