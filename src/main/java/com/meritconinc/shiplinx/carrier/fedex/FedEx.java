...
//		log.debug("------------------rateShipment----------------");
	...
	public void shipOrder(ShippingOrder order, Rating rate, CustomerCarrier customerCarrier) throws FedExException {
		...
		if(order.getPackages().size()>1)
		{
			...
			order.getPackages().get(0).getLabelInfo().setLabelType(ShiplinxConstants.LABEL_TYPE_PDF_STRING);
			...
			for(int x=1;x<order.getPackages().size();x++)
			{
				...
				request.getRequestedShipment().getRequestedPackageLineItems(0).setWeight(new Weight(WeightUnits.LB, new BigDecimal(order.getPackages().get(x).getWeight())));
		    	request.getRequestedShipment().getRequestedPackageLineItems(0).setInsuredValue(new Money(shipWebServiceClient.getCurrency(), new BigDecimal(order.getPackages().get(x).getInsuranceAmount())));		    
				...
				order.getPackages().get(x).getLabelInfo().setLabelType(ShiplinxConstants.LABEL_TYPE_PDF_STRING);
