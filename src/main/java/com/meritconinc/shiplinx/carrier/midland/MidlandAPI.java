	...
	private List<Rating> rateShipmentByService(ShippingOrder order, Long serviceId, CustomerCarrier customerCarrier) throws CarrierException{
		...
		try {
			...
			for(com.meritconinc.shiplinx.model.Package p: order.getPackages()){
				...
				if(dimWeight < p.getWeight())
					totalWeightAir += p.getWeight();
				else
				...
				if(dimWeightGrd < p.getWeight())
					totalWeightGround += p.getWeight();
				else
...
//		boolean isCanceled=request_builder.createAndSendVoidRequest(order);	
	...
	public void generateShippingLabel(OutputStream outputStream, long orderId, CustomerCarrier customerCarrier) {
		...
		logger.error("Enter point");
		logger.error("Property 5"+customerCarrier.getProperty5());
	...
	public void generateShippingLabel(OutputStream outputStream, ShippingOrder order, CustomerCarrier customerCarrier) {
		...
		try{
			//InputStream stream = this.getClass().getClassLoader().getResourceAsStream("./jasperreports/src/main/java/com/meritconinc/shiplinx/carrier/generic/jasperreports/Generic_BOL.jasper");				
			InputStream stream=this.getClass().getClassLoader().getResourceAsStream("./jasperreports/src/main/java/com/meritconinc/shiplinx/carrier/generic/jasperreports/Courier.jasper");
			if(stream==null) {
			...
			Business business = businessDAO.getBusiessById(order.getBusinessId());
			logger.error("ServiceName()"+order.getService().getName());
            logger.error("ServiceCode()"+order.getService().getCode());
            logger.error("Reference1"+order.getReferenceOne());
            logger.error("bbbbb"+customerCarrier.getId());
            logger.error("CarrierId"+customerCarrier.getCarrierId());
            logger.error("CarrierAccountId"+customerCarrier.getCarrierAccountId());
            logger.error("BusinessId"+customerCarrier.getBusinessId());
            logger.error("Country"+customerCarrier.getCountry());
            logger.error("customerCarrier"+customerCarrier);
            int quantity=order.getQuantity(); 
            logger.error("quantity is "+quantity);
            String customerCarrierProperty5=customerCarrier.getProperty5();
            long wayBillNumber = Integer.parseInt(customerCarrierProperty5);
            logger.error(" wayBillNumber is "+wayBillNumber);
            //since we are in need of 001, 002 and so on
            long packageIDSuffix = wayBillNumber*1000;
            logger.error(" packageIDSuffix is "+packageIDSuffix);
            for(int i=1;i<=quantity;i++){
            	long packageIdWithoutPrefix = packageIDSuffix+i;
            	logger.error(" packageIdWithoutPrefix is "+packageIdWithoutPrefix);
            	listOfPackageID.add(customerCarrier.getAccountNumber1()+packageIdWithoutPrefix);
            	  logger.error("packageIDArray[i]"+listOfPackageID.get(i-1));
            }
            
            logger.error("AccountNumber1"+customerCarrier.getAccountNumber1());
            logger.error("Carrier Nmae"+order.getCarrier().getName());           
            //String barcodeGenerateNumber=customerCarrier.getAccountNumber1()+customerCarrierProperty5+barcodeSuffix;
            ...
            logger.error("customerId"+carrierAccountId);
            customerDAO.editCustomerCarrier(customerCarrier,carrierAccountId);
