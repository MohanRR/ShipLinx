...
public class ShippingOrder implements Serializable {
	...
	public ShippingOrder() {
		scheduledShipDate_web = FormattingUtil.getFormattedDate(new Date(), FormattingUtil.DATE_FORMAT_WEB);
	}
...
//	private String carrierPickUpConf;
	...
	private double dutiableAmount = 0;
	...
	private Double weightToMarkup = null; //used during the rating process when we do not know which rate/service will be selected
...
//	}
	...
	/**
	 * @param packageArray the packageArray to set
	 */
...
//	}
	...
	public String getBillingStatusText() {
		/*if (billingStatus != null && billingStatus.intValue()>=0 && 
				billingStatus.intValue() < ShiplinxConstants.BILLING_STATUS_LIST.length) {
			return ShiplinxConstants.BILLING_STATUS_LIST[billingStatus];
			
		}*/
		if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_NOT_INVOICED)
			return ShiplinxConstants.BILLING_STATUS_NOT_INVOICED_TEXT;
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_AWAITING_CONFIRMATION)
			return ShiplinxConstants.BILLING_STATUS_AWAITING_CONFIRMATION_TEXT;
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_CUSTOMER_CONFIRMATION)
			return ShiplinxConstants.BILLING_STATUS_CUSTOMER_CONFIRMATION_TEXT;
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_ORPHAN)
			return ShiplinxConstants.BILLING_STATUS_ORPHAN_TEXT;
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_READY_TO_INVOICE)
			return ShiplinxConstants.BILLING_STATUS_READY_TO_INVOICE_TEXT;
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_INVOICED)
			return ShiplinxConstants.BILLING_STATUS_INVOICED_TEXT;			
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_THIRD_PARTY)
			return ShiplinxConstants.BILL_TO_THIRD_PARTY;			
		else if(billingStatus!=null && billingStatus.intValue() == ShiplinxConstants.BILLING_STATUS_COLLECT)
	...
	public void setBillingStatusText(String status) {
		if (!StringUtil.isEmpty(status)) {
			/*for (int i=0; i < ShiplinxConstants.BILLING_STATUS_LIST.length; i++) {
				String s = ShiplinxConstants.BILLING_STATUS_LIST[i];
				if (s.equals(status)) {
					billingStatus = i;
					return;
				}
	...
	public double getTotalCostQuoted(){
		return getTotalCost(new int[]{ShiplinxConstants.CHARGE_QUOTED});
	}

	public double getTotalChargeActual(){
		return getTotalCharge(new int[]{ShiplinxConstants.CHARGE_PENDING_RELEASE, ShiplinxConstants.CHARGE_READY_TO_INVOICE,ShiplinxConstants.CHARGE_INVOICED});
	}

	public double getTotalCostActual(){
		return getTotalCost(new int[]{ShiplinxConstants.CHARGE_PENDING_RELEASE, ShiplinxConstants.CHARGE_READY_TO_INVOICE,ShiplinxConstants.CHARGE_INVOICED});
	}
...
//			case ShiplinxConstants.STATUS_DISPATCHED: return MessageUtil.getMessage("status.rfs"); //name changed from "dispatched" to "rfs"
...
//			case ShiplinxConstants.STATUS_READYTOPROCESS: return MessageUtil.getMessage("status.ready_to_process");
//			case ShiplinxConstants.STATUS_SENT_TOWAREHOUSE: return MessageUtil.getMessage("status.sent.warehouse");
//			case ShiplinxConstants.STATUS_RECEIVED_BYWAREHOUSE: return MessageUtil.getMessage("status.received.warehouse");
//			case ShiplinxConstants.STATUS_SHIPPED: return MessageUtil.getMessage("status.shipped");
//		}
//		return "";
//	}
	public double getDutiableAmount() {
		return dutiableAmount;
	}
	public void setDutiableAmount(double dutiableAmount) {
		this.dutiableAmount = dutiableAmount;
	}
