				...
				if (custMarkupList != null) {
					...
					for(Markup m:custMarkupList) {
						...
						if (n == -1) {
							...
							defMarkupList.remove(n);	// There is already a customized markup, therfore remove default Markup
...
//			} else {
//				throw new MarkupAlreadyExistsException("Markup already exists - " + mList.get(0).getServiceName());
//			}
//		}
//	}
...
//			// incase
	...
	public Markup addMarkup(Markup markup){
		...
		if (markupDAO != null && markup != null) {
			...
			if (m == null || 
				m.getCustomerId().longValue() != markup.getCustomerId().longValue() ||
				!m.getFromCountryCode().equals(markup.getFromCountryCode()) ||
				!m.getToCountryCode().equals(markup.getToCountryCode()) ||
	...
	public Double applyMarkup(ShippingOrder shipment, Charge charge, boolean setShipmentMarkup) {
		...
		if (shipment != null && charge != null) {
			if (shipment.getMarkPercent() == null) {
				...
				if(markup == null){//if the markup is null (could not find mark up object for this shipment/charge), return the cost
					log.warn("Could not find mark up record for business/customer/service : " + searchMarkup.getBusinessId() + " / " + searchMarkup.getCustomerId() + " / " + searchMarkup.getServiceId());
					return charge.getCost();
				}
				markPerc = markup.getMarkupPercentage();
				markType = markup.getType();

			}
			else{ //already set in the shipment
				markPerc = shipment.getMarkPercent();
				markType = shipment.getMarkType();
			}
			...
			if (markPerc >= 0) {
				if (markType > 0) {
					if (markType == ShiplinxConstants.TYPE_MARKDOWN) {
						//after introduction of static component, we remove the static component before calculating the marked up or down value
						double f = ((FormattingUtil.subtract(charge.getTariffRate().doubleValue(), charge.getStaticAmount())).doubleValue()) * markPerc / 100;
						amount = (FormattingUtil.subtract(charge.getTariffRate().doubleValue(), f)).doubleValue();
					} else { //marking up
						double f = (FormattingUtil.subtract(charge.getCost().floatValue(), charge.getStaticAmount())).doubleValue() * markPerc / 100;
	...
	private Markup applyRules(List<Markup> mList, Markup markup) {
		...
		for (Markup m:mList) {
			...
			if (m.getServiceId().equals(markup.getServiceId()) && isCompareCountries(m, markup) && m.getFromWeight().floatValue() == 0) {
				if (bCus && m.getCustomerId().equals(markup.getCustomerId()))
					return m;
				if (m.getCustomerId().longValue() == 0)
					return m;
...
//	}
	...
	public void copyCustomerMarkup(Long sourceCusId, Long targetCusId, long businessId) {
		// TODO Auto-generated method stub
		if (sourceCusId != null && targetCusId != null && sourceCusId.longValue() != targetCusId.longValue()) {
	...
	public Service uploadRateTemplateFile(Service service, Long customerId, Long busId, 
		...
		try {
			...
			if (service != null && service.getCarrierId() > 0 && uploadFileName != null&& uploadStream != null) {
				if (uploadFileName.toUpperCase().endsWith(ShiplinxConstants.CSV_EXTENSION)) {
					String uploadFolder = WebUtil.getProperty(Constants.SYSTEM_SCOPE, "BASE_PATH") + File.separator + Constants.RATE_TEMPLATE_FOLDER;
					File outFile = new File(uploadFolder + File.separator + ShiplinxConstants.IN_FOLDER + File.separator + uploadFileName);
	...
	private synchronized void beginProcessing(long ltlServicecId, long ltlCustomerId, long ltlBusId, boolean isOverwrite){
		try {
			if (this.carrierServiceManager == null) {
				setCarrierServiceManager((CarrierServiceManager) MmrBeanLocator.getInstance().findBean("carrierServiceManager"));
			}		
