...
public class AddressManagerImpl implements AddressManager{
	...
	private static final Logger log = LogManager.getLogger(AddressManagerImpl.class);
	...
	public void parseFile(String customerId, InputStream data, String name, boolean deleteExistingAddressess) throws ShiplinxException{
		try{
			...
			String[][] values = null;
			try {
				values = new LabeledCSVParser(new ExcelCSVParser(br)).getAllValues();
			} catch (IOException e) {
			...
			for (int i=0; i<values.length; i++){
				...
				if(values[i].length != 12){
					throw new ShiplinxException("Invalid file format at Line "+(i+1));
				}
				...
				for (int j=0; j<values[i].length; j++){
					...
					if(j == 11) {
						if(values[i][j]!=null && values[i][j].equalsIgnoreCase("Y"))
	...
	public void runAddressValidation(Address address){
		try{
			...
			if(!address.getCountryCode().equalsIgnoreCase(ShiplinxConstants.CANADA) && !address.getCountryCode().equalsIgnoreCase(ShiplinxConstants.US))
				return;
			...
			log.error("Could not validate address for postal and country: " + address.getPostalCode() + " / " + address.getCountryCode());
		}
		
	}

	/*
	 * This method retrieves the list of [cities, zipcode] from the database for provided city and country code.
	 * @param String, String
	 * @return List<String>
	 * @Exception Nothing.
	 */
	...
	public List<String> getCitySuggest(String country, String city){
		...
		DHLCanadaTariffDAO dhlCanadaTariffDAO = (DHLCanadaTariffDAO)context.getBean("dhlCanadaTariffDAO");
	...
	/*
	 * This method retrieves the list of [zipcode, cities] from the database for provided zipcode and country code.
	 * @param String, String
	 * @return List<String>
	 * @Exception Nothing.
	 */
