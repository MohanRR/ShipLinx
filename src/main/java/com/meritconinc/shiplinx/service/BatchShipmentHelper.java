...
public class BatchShipmentHelper {
	...
	private void parseRecord(String[] fields, String[] rowData, List<ShippingOrder> shipments) {
		...
		try {
			...
			so.setDutiableAmount(StringUtil.getDouble(rowData[Dutiable_amount]));
			...
			p.setWeight(Float.parseFloat(rowData[Package_Weight])); 			
			p.setInsuranceAmount(Float.parseFloat(rowData[Package_Insurance_Amount])); 	
			p.setDescription(rowData[Package_Description]);
			p.setCodAmount(Float.parseFloat(rowData[Package_COD_Amount]));
